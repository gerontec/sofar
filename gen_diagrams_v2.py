...
digraph Architecture {
    graph [
        label="fox2db.py v2.1-Py — Schichten-Architektur (prozedural, 569 Zeilen)"
    ...
    do4       -> Relay    [color="#c0392b" label="DO4"]
...
digraph MainFlow {
    graph [
        label="fox2db.py v2.1-Py — main() Ablauf"
        labelloc=t fontsize=14 fontname="Helvetica-Bold"
        rankdir=TB splines=polyline nodesep=0.4 ranksep=0.55
        bgcolor="#f8f9fa" size="11,17" ratio=fill
    ]
    ...
    s8  [shape=box fillcolor="#f8d7da" label="apply_guards(best, soc)\n→ BATTERY_FULL_STOP (soc>99 → 0)\n→ CRITICAL_SOC      (soc<6  → 1)"]
    ...
    s11 [shape=box fillcolor="#cce5ff" label="_log(Data + Result + TRACE)"]
    s12 [shape=box fillcolor="#cce5ff" label="publish_mqtt() → fox2db/state"]
    ...
    s15 [shape=box fillcolor="#cce5ff" label="_db_decision_log() → pv_decision_log"]
    ...
    d4  [shape=diamond fillcolor="#fce8e8" label="pcc >20000W\nund trace nicht\nPCC_OVER_20KW?"]
    s17 [shape=box fillcolor="#f8d7da" label="pulse_do4()\n(SOC=100 oder State=7\n→ kein State+1 möglich)"]
...
digraph DecisionDetail {
    graph [
        label="fox2db.py v2.1-Py — Decision Layer im Detail"
        labelloc=t fontsize=14 fontname="Helvetica-Bold"
        rankdir=TB splines=polyline nodesep=0.4 ranksep=0.55
        bgcolor="#f8f9fa" size="11,17" ratio=fill
    ]
    ...
    d_pcc20     -> d_soc_unk [label="NEIN" color="green"]
    r_pcc20     -> End
